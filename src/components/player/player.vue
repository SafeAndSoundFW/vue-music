...
export default {
  ...
  methods: {
    ...
    middleTouchMove(e) {
      ...
      // 取绝对值，如果在纵轴滚动的偏差大于 横轴滚动的偏差，不做处理
      if (Math.abs(deltaY) > Math.abs(deltaX)) {
        return
      }
      const left = this.currentShow === 'cd' ? '0' : -window.innerWidth
      const offsetWidth = Math.min(0, Math.max(-window.innerWidth, Number(left) + Number(deltaX)))
    ...
    _getPosAndScale() {
      const targetWidth = 40 //小圆圈的大小
      const paddingLeft = 40 //小中心店距左边的大小
      const paddingBottom = 30 //小中心店距底部的大小
      const paddingTop = 80 //唱片容器距顶部的大小
  ...
  watch: {
    currentSong(newSong, oldSong) {
      ...
      // })
      // 微信内切到后台，这首歌还是要播完的，为了解决从后台切到前台不能正常播放，改为定时器
      setTimeout(() => {
        this.$refs.audio.play()
        this.getLyric()
